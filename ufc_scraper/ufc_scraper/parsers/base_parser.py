"""Defines the abstract base parser class - inherited by event/fight/fighter/fight stat parsers."""
...
class Parser(ABC):
    """Abstract base class for parsing web responses.

    This class provides the foundational structure for parsing logic, including
    initialization of common attributes (URL, ID) and helper methods for
    safe data extraction using CSS selectors and XPath.

    Attributes:
        _response (Response): The HTTP response object to be parsed.
        _url (str): The URL extracted from the response.
        _id (str): A unique UUID generated based on the URL.
        _css_queries (CssQueries): An instance containing reusable CSS selector strings.
    ...
    def parse_response(self) -> Any:
        """Abstract method to define specific parsing logic.

        Subclasses must implement this method to extract and structure data
        from the response.
        """
        pass
